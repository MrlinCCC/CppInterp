...
TimerHandle Timer::RunAt(std::chrono::steady_clock::time_point time, TimerTask::TimerCallback cb)
{
    TimerTask task;
    task.m_id = GenerateAutoIncrementId<Timer>();
    task.m_canceled = std::make_shared<std::atomic<bool>>(false);
    task.m_expireTime = time;
    task.m_cb = cb;
    AddTask(task);

    return TimerHandle(task.m_id, task.m_canceled);
}

TimerHandle Timer::RunAfter(long long delayMs, TimerTask::TimerCallback cb)
{
    TimerTask task;
    task.m_id = GenerateAutoIncrementId<Timer>();
    task.m_canceled = std::make_shared<std::atomic<bool>>(false);
...
TimerHandle Timer::RunEvery(long long intervalMs, TimerTask::TimerCallback cb)
{
    TimerTask task;
    task.m_id = GenerateAutoIncrementId<Timer>();
    task.m_canceled = std::make_shared<std::atomic<bool>>(false);
...
void Timer::Cascade(std::shared_ptr<TimerWheel> wheel)
{
    ...
    std::list<TimerTask> slot;
    {
        ...
        slot.swap(wheel->m_slots[curIndex]);
    }
...
{
    {
        ...
        std::list<TimerTask> slot;
        {
            ...
            slot.swap(p_headWheel->m_slots[curIndex]);
        }

        auto now = std::chrono::steady_clock::now();
        for (auto it = slot.begin(); it != slot.end();)
        {
            if (it->m_canceled && *it->m_canceled)
            {
                it = slot.erase(it);
                continue;
            }
            if (now >= it->m_expireTime)
            {
                ...
                if (it->m_isEvery && !(it->m_canceled && *it->m_canceled))
                {
